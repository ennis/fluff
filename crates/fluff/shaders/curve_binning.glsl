...
layout(set=0,binding=2,r32i) uniform coherent iimage2D tileLineCountImage;
layout(scalar,set=0,binding=3) buffer TileBuffer {
    Tile tiles[];
};
...
void main() {
    ...
    int count = imageAtomicAdd(tileLineCountImage, tilePos, 1);
    if (count < MAX_LINES_PER_TILE) {
        tiles[tileIndex].entries[count].line = i_line;
        tiles[tileIndex].entries[count].curveIndex = i_curveIndex;
    }
