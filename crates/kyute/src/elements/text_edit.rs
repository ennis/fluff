...
impl TextEdit {
    ...
    fn set_selection_inner(&mut self, selection: Selection) -> bool {
    ...
    fn set_cursor_at_text_position(&mut self, pos: usize, keep_anchor: bool) -> bool {
        self.set_selection_inner(if keep_anchor {
            Selection {
    ...
    fn move_cursor_to_next_word_inner(&mut self, keep_anchor: bool) -> bool {
        let end = next_word_boundary(&self.text, self.selection.end);
        self.set_cursor_at_text_position(end, keep_anchor)
    }

    fn move_cursor_to_prev_word_inner(&mut self, keep_anchor: bool) -> bool {
        let end = prev_word_boundary(&self.text, self.selection.end);
        self.set_cursor_at_text_position(end, keep_anchor)
    }

    fn move_cursor_to_next_grapheme_inner(&mut self, keep_anchor: bool) -> bool {
        let end = next_grapheme_cluster(&self.text, self.selection.end).unwrap_or(self.selection.end);
        self.set_cursor_at_text_position(end, keep_anchor)
    }

    fn move_cursor_to_prev_grapheme_inner(&mut self, keep_anchor: bool) -> bool {
    ...
    fn text_position_for_point_inner(&self, point: Point) -> usize {
    ...
    fn set_cursor_at_point_inner(&mut self, point: Point, keep_anchor: bool) -> bool {
        let point = point + self.scroll_offset;
    ...
    fn select_word_under_cursor_inner(&mut self) -> bool {
        let selection = self.selection;
        let range = self.paragraph.get_word_boundary(selection.end as u32);
        self.set_selection_inner(Selection {
            start: range.start,
            end: range.end,
        })
    }

    fn word_selection_at_text_position_inner(&self, pos: usize) -> Selection {
    ...
    fn select_line_under_cursor_inner(&mut self) -> bool {
        let text = &self.text;
        let selection = self.selection;
        ...
        self.set_selection_inner(Selection { start, end })
    }
}
...
impl TextEdit {
    ...
    pub fn text_position_for_point(&self, point: Point) -> usize {
        self.text_position_for_point_inner(point)
    }
    ...
    pub fn word_selection_at_text_position(&self, pos: usize) -> Selection {
        self.word_selection_at_text_position_inner(pos)
    }
...
impl Element for TextEdit {
    ...
    fn paint(self: &mut ElemBox<Self>, ctx: &mut PaintCtx) {
        ...
        ctx.save();
        ctx.canvas().translate(-self.scroll_offset.to_skia());
