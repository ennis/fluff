...
use crate::layout::{Alignment, BoxConstraints, Geometry, LengthOrPercentage};
...
impl Default for GridItemOptions {
    fn default() -> Self {
        GridItemOptions {
...
fn grid_layout(grid: &GridOptions, insets: &Insets, children: &[Rc<dyn ElementMethods>], constraints: &BoxConstraints) -> GridLayout {
    let _span = span!("Grid layout");

    // apply style insets
    let constraints = constraints.deflate(*insets);
    ...
    let mut grid_items = vec![GridItemCalculated::default(); children.len()];
    let (row_count, column_count) = place_grid_items(grid, children, &mut grid_items);

    // lay out the columns first
    let (width, _width_changed) = layout_tracks(
        &constraints,
        GridAxis::Column,
        column_count,
...
/// Calculated values for grid items.
#[derive(Default)]
struct GridItemCalculated {
...
fn place_grid_items(grid: &GridOptions, children: &[Rc<dyn ElementMethods>], items: &mut [GridItemCalculated]) -> (usize, usize) {
    ...
    let mut final_row_count = grid.rows.len();
    let mut final_column_count = grid.columns.len();

    let mut flow_cursor = FlowCursor {
        row: 0,
        column: 0,
        flow_dir_size: match grid.flow {
            FlowDirection::Row => grid.columns.len() as u32,
            FlowDirection::Column => grid.rows.len() as u32,
        },
        flow: grid.flow,
    };

    for item in items.iter_mut() {
        if item.options.area.is_null() {
        ...
        let (row_range, column_range) = flow_cursor.place(item.options.area);
