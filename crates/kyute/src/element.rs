...
use taffy::{Cache, LayoutInput, LayoutOutput, NodeId, RequestedAxis, RunMode, TraversePartialTree};
...
pub struct Element {
    ...
    /// Taffy layout cache.
    pub(crate) cache: RefCell<taffy::Cache>,
    /// Taffy layout styles.
    pub(crate) layout_styles: taffy::Style,
...
impl Element {
    ...
    pub fn child_at(&self, index: usize) -> Option<Rc<dyn ElementMethods>> {
        ...
        unsafe {
            self.children_ref().get(index).cloned()
        }
    }
    ...
    pub fn get<T: AttachedProperty>(&self) -> Option<T::Value> {
        ...
        attached_properties.get(&TypeId::of::<T>()).map(|v| {
    ...
    pub unsafe fn get_ref<T: AttachedProperty>(&self) -> &T::Value {
        self.try_get_ref::<T>().expect("attached property not set")
    }

    pub unsafe fn try_get_ref<T: AttachedProperty>(&self) -> Option<&T::Value> {
        let attached_properties = unsafe { &*self.attached_properties.get() };
        attached_properties
            .get(&TypeId::of::<T>())
...
pub trait ElementMethods: EventTarget {
    ...
    fn layout(&self, children: &[Rc<dyn ElementMethods>], layout_input: &LayoutInput) -> LayoutOutput {
        // The default implementation just returns the union of the measures of the children.
        let mut width: f32 = 0.0;
        let mut height: f32 = 0.0;
        for child in children {
            let child_layout = child.do_layout(&layout_input);
            width = width.max(child_layout.size.width);
            height = height.max(child_layout.size.height);
...
impl dyn ElementMethods + '_ {
    ...
    pub fn do_layout(&self, input: &LayoutInput) -> LayoutOutput {
        let children = self.children();

        let layout = self.layout(&*children, input);
        ...
        if input.run_mode == taffy::RunMode::PerformLayout {
            //assert!(input.known_dimensions.both_axis_defined());
            self.geometry.set(Size::new(layout.size.width as f64, layout.size.height as f64));
        }
